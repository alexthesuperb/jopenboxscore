...
public class Boxscore {
    private static String pColumns = String.format("%3s%3s%3s%3s%3s%3s",
        "IP","H","R","ER","BB","SO");
    ...
    public static void printBoxscore(BufferedWriter writer, Team visitor, Team home,
        String date, char dayNight, int gmNumber, int timeOfGame, int attend,
        int outs) throws IOException
    {
        //Print headline 
        String headline = "Game of " + date + " -- " + visitor.getCity() + " at " + home.getCity()
            + " (" + dayNight + ")";
        if(gmNumber > 0)
    ...
    /* Print inning run totals for each team, indenting each 3 innings. If game ends mid-inning
    (either on a walk off or on weather delay), indicate the number of outs recorded in that half-
    inning.*/
    private static void printLinescore(BufferedWriter writer, Team visitor, Team home, int outs) 
        throws IOException
    {
        ...
        writer.write(String.format("%-17s",visitor.getCity()) + v + " --" + 
            String.format("%3d",visitor.getTotalRuns()));
        writer.write("\n");
        writer.write(String.format("%-17s",home.getCity()) + h + " --" + 
            String.format("%3d",home.getTotalRuns()));
        writer.write("\n");
        if(outs < 3){
    ...
    /* Print each team's batting boxscore lines in parallel columns. Below each team's
    batting column, print its totals. */
        ...
        throws IOException
    {
        ...
        for(LinkedList<PositionPlayer> ar : visitor.getLineup()){
            for(PositionPlayer p : ar){
                visLineup.add(getBoxscoreLine(p));
            }
        }
        for(LinkedList<PositionPlayer> ar : home.getLineup()){
            for(PositionPlayer p : ar){
        ...
        battingLines.add(
            ...
            String.format("%4s","") + String.format("%-18s",home.getCity()) + 
            String.format("%3s%3s%3s%4s","AB","R","H","RBI")
        );
        for(int i = 0; i < max; i++){
    ...
    /* Example: "2B: Kiermaier, Pham 2, Stanton." */
    private static String getSpecialStatString(String stat_code, Team visitor, Team home,
        boolean isBattingStat)
    {
        String str = "";
        if(isBattingStat || stat_code.equals("pb")){
            for(LinkedList<PositionPlayer> ar : visitor.getLineup()){
                for(PositionPlayer p : ar){
                    if(p.get_stat(stat_code) > 0){
                        if(p.get_stat(stat_code) == 1)
                            str += p.getBoxscoreName();
                        else
                            str += p.getBoxscoreName() + " " + p.get_stat(stat_code);
                        str += ", ";
                    }
                }
            }
            ...
            if(stat_code.equals("hp") || stat_code.equals("hbp")){
                for(Pitcher p : visitor.getPitchingStaff()){
                    if(p.get_batters_hit().size() > 0){
                        str += "by " + p.getBoxscoreName() + " (";
                        for(String id : p.get_batters_hit()){
                            str += home.getPositionPlayer(id).getBoxscoreName() + ", ";
                        }
                        str = str.substring(0,str.length()-2);
                        str += "), ";
                    }
                }
                for(Pitcher p : home.getPitchingStaff()){
                    if(p.get_batters_hit().size() > 0){
                        str += "by " + p.getBoxscoreName() + " (";
                        for(String id : p.get_batters_hit()){
                            str += visitor.getPositionPlayer(id).getBoxscoreName() + ", ";
                        }
                        str = str.substring(0,str.length()-2);
                        str += "), ";
                    }
    ...
    private static void printAdditionalInfo(BufferedWriter writer, Team visitor, Team home)
        throws IOException
    {
        ...
        throws IOException
    {
        ...
        writer.write(
            String.format("%2s%-20s","",visitor.getCity()) + pColumns + "\n"
        );
        for(Pitcher p : visitor.getPitchingStaff())
            writer.write(getBoxscoreLine(p) + "\n");
        writer.write("\n");
        writer.write(
            String.format("%2s%-20s","",home.getCity()) + pColumns + "\n"
        );
        for(Pitcher p : home.getPitchingStaff())
            writer.write(getBoxscoreLine(p) + "\n");
    ...
    /* Return input pitcher's boxscore line:
        name (decision), IP, H, R, ER, BB, SO */
    private static String getBoxscoreLine(Pitcher p){
        String s1 = p.getBoxscoreName();
        char decision = p.getDecision();
        if(decision != '\0')
            s1 += " (" + decision + ")";
        if(p.removed_without_recorded_out_in_inning()){
            String c = String.valueOf(symbols.pop());
            s1 += c;
            String tmp = c + " Pitched to " + p.getInningBattersFaced() + " ";
            tmp += (p.getInningBattersFaced() == 1) ? "batter" : "batters";
            tmp += " in ";
            if(p.get_inning_removed() == 1){
                tmp += "1st";
            } else if (p.get_inning_removed() == 2){
                tmp += "2nd";
            } else if (p.get_inning_removed() == 3){
                tmp += "3rd";
            } else {
                tmp += p.get_inning_removed() + "th";
            }
            pitching_info_strings.add(tmp);
        }
        int[] pitching_stats = p.getPitchingStats();
    ...
    /* Return input batter's boxscore line:
        name, AB, R, H, RBI */
    private static String getBoxscoreLine(PositionPlayer p){
        String s1 = p.getBoxscoreName() + ", " + p.getPositionString();
        int[] batting_stats = p.getBattingStats();
